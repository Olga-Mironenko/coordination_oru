...
public abstract class TrajectoryEnvelopeCoordinator extends AbstractTrajectoryEnvelopeCoordinator {
    ...
    protected HashMap<Integer, CoordinatorDependency> replanningStoppingPoints = new HashMap<Integer, CoordinatorDependency>();
    ...
    protected List<List<Integer>> findSimpleNonliveCycles(SimpleDirectedGraph<Integer, CoordinatorDependency> g) {

        JohnsonSimpleCycles<Integer, CoordinatorDependency> cycleFinder = new JohnsonSimpleCycles<Integer, CoordinatorDependency>(g);
        ...
        for (List<Integer> cycle : cycles) {
            ...
            ArrayList<CoordinatorDependency> edgesAlongCycle = new ArrayList<CoordinatorDependency>();
            for (int i = 0; i < cycle.size(); i++) {
                CoordinatorDependency dep = g.getEdge(cycle.get(i), cycle.get(i < cycle.size() - 1 ? i + 1 : 0));
    ...
    protected SimpleDirectedGraph<Integer, CoordinatorDependency> depsToGraph(HashMap<Integer, CoordinatorDependency> deps) {
        SimpleDirectedGraph<Integer, CoordinatorDependency> g = new SimpleDirectedGraph<Integer, CoordinatorDependency>(CoordinatorDependency.class);
        for (int key : deps.keySet()) {
            CoordinatorDependency dep = deps.get(key);
    ...
    protected HashMap<Integer, CoordinatorDependency> computeClosestDependencies(HashMap<Integer, HashSet<CoordinatorDependency>> allDeps, HashMap<Integer, HashSet<CoordinatorDependency>> artificialDeps) {

        HashMap<Integer, CoordinatorDependency> closestDeps = new HashMap<Integer, CoordinatorDependency>();
        ...
        //Fill the TreeSet
        HashMap<Integer, TreeSet<CoordinatorDependency>> allDepsTree = new HashMap<Integer, TreeSet<CoordinatorDependency>>();
        for (Integer key : allDeps.keySet()) allDepsTree.put(key, new TreeSet<CoordinatorDependency>(allDeps.get(key)));
        HashMap<Integer, TreeSet<CoordinatorDependency>> artificialDepsTree = new HashMap<Integer, TreeSet<CoordinatorDependency>>();
        for (Integer key : artificialDeps.keySet())
            artificialDepsTree.put(key, new TreeSet<CoordinatorDependency>(artificialDeps.get(key)));
        ...
        for (Integer robotID : robotIDs) {
            if (allDeps.containsKey(robotID) || artificialDeps.containsKey(robotID)) {
                CoordinatorDependency firstDep = null;
                CoordinatorDependency firstArtificialDep = null;
                ...
                CoordinatorDependency depToSend = null;
                if (firstDep != null)
    ...
    public boolean isDeadlocked() {
        synchronized (solver) {
            SimpleDirectedGraph<Integer, CoordinatorDependency> g = depsToGraph(currentDependencies);
    ...
    protected HashMap<Integer, HashSet<CoordinatorDependency>> findAndRepairNonliveCycles(HashMap<Integer, HashSet<CoordinatorDependency>> currentDeps, HashMap<Integer, HashSet<CoordinatorDependency>> artificialDeps, HashSet<CoordinatorDependency> reversibleDeps, HashMap<Integer, RobotReport> currentReports, Set<Integer> robotIDs) {

        //Create a copy of the current set of dependencies
        HashMap<Integer, HashSet<CoordinatorDependency>> allDeps = new HashMap<Integer, HashSet<CoordinatorDependency>>();
        for (int robotID : currentDeps.keySet()) {
            HashSet<CoordinatorDependency> set = new HashSet<CoordinatorDependency>();
        ...
        if (breakDeadlocksByReordering)
            allDeps = callLocalReordering(nonliveCycles, artificialDeps, g, reversibleDeps, allDeps, currentReports);
    ...
    protected HashMap<Integer, HashSet<CoordinatorDependency>> callLocalReordering(List<List<Integer>> nonliveCycles, HashMap<Integer, HashSet<CoordinatorDependency>> artificialDeps, SimpleDirectedGraph<Integer, CoordinatorDependency> g, HashSet<CoordinatorDependency> reversibleDeps, HashMap<Integer, HashSet<CoordinatorDependency>> allDeps, HashMap<Integer, RobotReport> currentReports) {
        ...
        while (counter < nonliveCycles.size()) { //if empty no problem
            ...
            //Get edges along the cycle...
            ArrayList<CoordinatorDependency> reversibleDepsAlongCycle = new ArrayList<CoordinatorDependency>();
            for (int i = 0; i < cycle.size(); i++) {
                CoordinatorDependency dep = null;
                if (i < cycle.size() - 1) {
            ...
            for (CoordinatorDependency dep : reversibleDepsAlongCycle) {
                ...
                if (depsToCS.containsKey(dep)) {
                    ...
                    CoordinatorDependency revDep = new CoordinatorDependency(waitingTE, drivingTE, waitingPoint, drivingCSEnd);

                    HashMap<Integer, HashSet<CoordinatorDependency>> allDepsTmp = new HashMap<Integer, HashSet<CoordinatorDependency>>(allDeps);

                    //and add the reversed dependency to the new waiting one
                    if (!allDepsTmp.containsKey(waitingRobotID))
                        allDepsTmp.put(waitingRobotID, new HashSet<CoordinatorDependency>());
                    ...
                    HashMap<CoordinatorDependency, CriticalSection> depsToCSTmp = new HashMap<CoordinatorDependency, CriticalSection>(depsToCS);
                    depsToCSTmp.remove(dep);
                    depsToCSTmp.put(revDep, cs);

                    //update currentDeps
                    HashMap<Integer, CoordinatorDependency> currentDepsTmp = computeClosestDependencies(allDepsTmp, artificialDeps);
                    SimpleDirectedGraph<Integer, CoordinatorDependency> gTmp = depsToGraph(currentDepsTmp);
    ...
    protected boolean callOnePathReplan(List<Integer> cycle, SimpleDirectedGraph<Integer, CoordinatorDependency> g) {
        ...
        for (int i = 0; i < cycle.size(); i++) {
            ...
            if (dep == null) {
                ...
                dep = new CoordinatorDependency(te, null, waitingPoint, 0);
		 ...
		 * 	SimpleDirectedGraph<Integer,Integer> g = new SimpleDirectedGraph(Integer.class);
			...
			---------------------------------------------------------------------------------------
		 */
        ConnectivityInspector<Integer, CoordinatorDependency> connInsp = new ConnectivityInspector<Integer, CoordinatorDependency>(g);
    ...
    protected void localCheckAndRevise() {
        ...
        synchronized (solver) {
            ...
            HashMap<Integer, HashSet<CoordinatorDependency>> currentDeps = new HashMap<Integer, HashSet<CoordinatorDependency>>();
            HashMap<Integer, HashSet<CoordinatorDependency>> artificialDependencies = new HashMap<Integer, HashSet<CoordinatorDependency>>();
            HashSet<CoordinatorDependency> currentReversibleDependencies = new HashSet<CoordinatorDependency>();
                    ...
                    if (stoppingPoints.containsKey(robotID)) {
                        ...
                        for (int i = 0; i < stoppingPoints.get(robotID).size(); i++) {
                            ...
                            if (robotReport.getPathIndex() <= stoppingPoint) {
                                CoordinatorDependency dep = new CoordinatorDependency(robotTracker.getTrajectoryEnvelope(), null, stoppingPoint, 0);
                    ...
                    if (robotTracker1 instanceof TrajectoryEnvelopeTrackerDummy || robotTracker2 instanceof TrajectoryEnvelopeTrackerDummy) {
                        ...
                        if (createAParkingDep) {
                            ...
                            if (!currentDeps.containsKey(waitingTracker.getTrajectoryEnvelope().getRobotID()))
                                currentDeps.put(waitingTracker.getTrajectoryEnvelope().getRobotID(), new HashSet<CoordinatorDependency>());
                            CoordinatorDependency dep = new CoordinatorDependency(waitingTracker.getTrajectoryEnvelope(), drivingTracker.getTrajectoryEnvelope(), waitingPoint, drivingCSEnd);
                        ...
                        //Robot 2 can stop before entering critical section, robot 1 can't --> robot 2 waits
                            ...
                            if (!canExitCriticalSection(drivingCurrentIndex, waitingCurrentIndex, drivingTracker.getTrajectoryEnvelope(), waitingTracker.getTrajectoryEnvelope(), lastIndexOfCSDriving)) {
                                ...
                                CoordinatorDependency dep = new CoordinatorDependency(drivingTracker.getTrajectoryEnvelope(), waitingTracker.getTrajectoryEnvelope(), Math.max(0, artWaitingPoint), artDrivingCSEnd);
                                if (!artificialDependencies.containsKey(drivingRobotID))
                                    artificialDependencies.put(drivingRobotID, new HashSet<CoordinatorDependency>());
            ...
            synchronized (currentDependencies) {
                ...
                HashMap<Integer, CoordinatorDependency> closestDeps = computeClosestDependencies(currentDeps, artificialDependencies);
                currentDependencies.clear();
    ...
    protected boolean setMaxCPDependencies(Set<Integer> robotsIDs) {
        synchronized (replanningStoppingPoints) {
            ...
            if (tryLocking) {
                HashMap<Integer, CoordinatorDependency> currentDeps = getCurrentDependencies();
                for (int robotID : robotsIDs) {
                    if (currentDeps.containsKey(robotID)) {
                        CoordinatorDependency dep = currentDeps.get(robotID);
                        replanningStoppingPoints.put(dep.getWaitingRobotID(), new CoordinatorDependency(dep.getWaitingTrajectoryEnvelope(), null, dep.getWaitingPoint(), 0));
                    }
                }
    ...
    protected boolean nonlivePair(CoordinatorDependency dep1, CoordinatorDependency dep2) {
        ...
        for (int robotID : robotsToReplan) {
            ...
            synchronized (getCurrentDependencies()) {
                ...
                if (robotsAsObstacles.size() > 0) {
                    ...
                    for (int otherRobotID : robotsAsObstacles)
                        if (otherRobotID != robotID) otherRobotIDs.add(otherRobotID);
                    if (!otherRobotIDs.isEmpty())
                        obstacles = getObstaclesInCriticalPoints(ArrayUtils.toPrimitive(otherRobotIDs.toArray(new Integer[otherRobotIDs.size()])));
                }
            }
        ...
        this.inference = new GatedThread("TrajectoryEnvelopeCoordinator inference") {
            ...
            public void runCore() {
                ...
                if (avoidDeadlockGlobally.get())
                    stat = stat + "\t numberReversedPrecedenceOrder\t numberCurrentCycles";
                stat.concat("\n");
                ...
                while (!stopInference) {
                    ...
                    if (CONTROL_PERIOD > 0) {
                        try {
                            GatedThread.sleep(expectedSleepingTime);
                    ...
                    if (avoidDeadlockGlobally.get())
                        stat = stat + "\t" + currentOrdersHeurusticallyDecided.get() + "\t" + currentCyclesList.size();
                    stat.concat("\n");
                    writeStat(fileName, stat);
                }
    ...
    protected void globalCheckAndRevise() {
        ...
        synchronized (solver) {
            ...
            HashMap<Integer, HashSet<CoordinatorDependency>> currentDeps = new HashMap<Integer, HashSet<CoordinatorDependency>>();
            HashMap<Integer, HashSet<CoordinatorDependency>> artificialDependencies = new HashMap<Integer, HashSet<CoordinatorDependency>>();
            DirectedMultigraph<Integer, CoordinatorDependency> depsGraph = new DirectedMultigraph<Integer, CoordinatorDependency>(CoordinatorDependency.class);
            ...
            synchronized (allCriticalSections) {
                ...
                for (CriticalSection cs : toRemove) {
                    ...
                    if (CSToDepsOrder.containsKey(cs)) {
                        ...
                        metaCSPLogger.info("WARNING: Obsolete critical section " + cs + " was not assigned to a dependency.");
                    this.escapingCSToWaitingRobotIDandCP.remove(cs);
                }
                ...
                for (CriticalSection cs : reversibleCS) {
                    ...
                    if (robot2YieldsOld != robot2Yields) {
                        ...
                        if (waitingPoint >= 0) {
                            ...
                            DirectedMultigraph<Integer, CoordinatorDependency> backupDepsGraph = new DirectedMultigraph<Integer, CoordinatorDependency>(CoordinatorDependency.class);
                            for (int v : depsGraph.vertexSet()) backupDepsGraph.addVertex(v);
                            for (CoordinatorDependency dep : depsGraph.edgeSet()) {
                            ...
                            CoordinatorDependency depNew = new CoordinatorDependency(waitingTracker.getTrajectoryEnvelope(), drivingTracker.getTrajectoryEnvelope(), waitingPoint, drivingCSEnd);
                            CoordinatorDependency depOld = new CoordinatorDependency(drivingTracker.getTrajectoryEnvelope(), waitingTracker.getTrajectoryEnvelope(), CSToDepsOrder.get(cs).getSecond(), drivingCSEndOld);
                            ...
                            if (currentCyclesList.containsKey(newEdge)) {
                                ...
                                for (List<Integer> cycle : currentCyclesList.get(newEdge)) {
                                    ...
                                    ArrayList<ArrayList<CoordinatorDependency>> edgesAlongCycle = new ArrayList<ArrayList<CoordinatorDependency>>();
                                    for (int i = 0; i < cycle.size(); i++) {
                                        int j = i < cycle.size() - 1 ? i + 1 : 0;
                                        if (cycle.get(i) == depNew.getWaitingRobotID() && cycle.get(j) == depNew.getDrivingRobotID()) {
                                            ArrayList<CoordinatorDependency> array = new ArrayList<CoordinatorDependency>();
                                            array.add(depNew);
                                            edgesAlongCycle.add(array);
                                        } else {
                                            Set<CoordinatorDependency> allEdges = depsGraph.getAllEdges(cycle.get(i), cycle.get(j));
                                            if (allEdges == null) {
                                                ...
                                                if (allEdges.isEmpty()) {
                                                    ...
                                                    if (currentDeps.containsKey(cycle.get(i))) {
                                                        for (CoordinatorDependency dep1 : currentDeps.get(cycle.get(i))) {
                                                ...
                                                edgesAlongCycle.add(new ArrayList<CoordinatorDependency>(allEdges));
                                            }
                                        }
                                    }
                                    ...
                                    int maxSize = 0;
                                    for (ArrayList<CoordinatorDependency> oneDepList : edgesAlongCycle)
                                    ...
                                    ArrayList<ArrayList<CoordinatorDependency>> newDeps = new ArrayList<ArrayList<CoordinatorDependency>>();
                                    for (int k = 0; k < v.length; k++) {
                                        int[] oneComb = v[k];
                                        ArrayList<CoordinatorDependency> oneSelection = new ArrayList<CoordinatorDependency>();
                                    ...
                                    for (ArrayList<CoordinatorDependency> depList : newDeps) {
                                        for (int i = 0; i < depList.size(); i++) {
                                            safe = true;
                                            CoordinatorDependency dep1 = depList.get(i);
                                            CoordinatorDependency dep2 = depList.get(i < depList.size() - 1 ? i + 1 : 0);
    ...
    protected void sendCriticalPoint(int robotID, HashMap<Integer, RobotReport> currentReports) {
        ...
        synchronized (currentDependencies) {
            if (currentDependencies.containsKey(robotID)) {
                CoordinatorDependency dep = currentDependencies.get(robotID);
