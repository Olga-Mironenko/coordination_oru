...
import se.oru.coordination.coordination_oru.simulator.FleetVisualization;
...
public abstract class AbstractTrajectoryEnvelopeCoordinator {
    ...
    HashMap<CoordinatorDependency, CriticalSection> depsToCS = new HashMap<CoordinatorDependency, CriticalSection>();
        ...
        if (shape1.intersects(shape2)) {
            ...
            for (int i = 0; i < allIntersections.size(); i++) {
                ...
                // critical sections around holes.
                    ...
                    if (te1Starts.size() == 0 || te2Starts.size() == 0)
                        System.out.println("CRAP: te1Starts is " + te1Starts + " and te2Starts is " + te2Starts);
        ...
        synchronized (solver) {
            ...
            TrackingCallback cb = new TrackingCallback(parking) {
                ...
                public String[] onPositionUpdate() {
                    if (trackingCallbacks.containsKey(robotID))
                        return trackingCallbacks.get(robotID).onPositionUpdate();
                    return null;
                }
            };
     ...
     *
     * @return A list of {@link CoordinatorDependency} objects.
     */
    public HashMap<Integer, CoordinatorDependency> getCurrentDependencies() {
    ...
    protected Geometry[] getObstaclesInCriticalPoints(int... robotIDs) {
        ...
        for (int robotID : robotIDs) {
            ...
            if (tracker instanceof TrajectoryEnvelopeTrackerDummy) { // the robot is parked
                ...
                CoordinatorDependency dep = currentDeps.containsKey(robotID) ? getCurrentDependencies().get(robotID) : null;
    ...
    protected Geometry[] getObstaclesFromWaitingRobots(int robotID) {
        ...
        ArrayList<Geometry> ret = new ArrayList<Geometry>();
        HashMap<Integer, CoordinatorDependency> currentDeps = getCurrentDependencies();
        CoordinatorDependency dep = currentDeps.containsKey(robotID) ? currentDeps.get(robotID) : null;
        if (dep != null) {
        ...
        synchronized (mp) {
            ...
            if (!replanningSuccessful) {
                try {
                    ...
                    System.out.println("Failed to write debug image");
                }
            }
        ...
        synchronized (solver) {
            for (final TrajectoryEnvelope te : envelopesToTrack) {
                ...
                TrackingCallback cb = new TrackingCallback(te) {
                    ...
                    public void onTrackingStart() {
                        if (trackingCallbacks.containsKey(myTE.getRobotID()))
                            trackingCallbacks.get(myTE.getRobotID()).onTrackingStart();
                    ...
                    public void onNewGroundEnvelope() {
                        if (trackingCallbacks.containsKey(myTE.getRobotID()))
                            trackingCallbacks.get(myTE.getRobotID()).onNewGroundEnvelope();
                    }
                    ...
                    public void beforeTrackingFinished() {
                        ...
                        if (trackingCallbacks.containsKey(myTE.getRobotID()))
                            trackingCallbacks.get(myTE.getRobotID()).beforeTrackingFinished();
                    }
                    ...
                    public void onTrackingFinished() {
                        ...
                        synchronized (solver) {
                            ...
                            if (trackingCallbacks.containsKey(myTE.getRobotID()))
                                trackingCallbacks.get(myTE.getRobotID()).onTrackingFinished();
                    ...
                    public String[] onPositionUpdate() {
                        ...
                        if (trackingCallbacks.containsKey(myTE.getRobotID()))
                            return trackingCallbacks.get(myTE.getRobotID()).onPositionUpdate();
                        return null;
                    }
        ...
        for (Entry<Integer, ArrayList<Mission>> e : robotsToMissions.entrySet()) {
            ...
            synchronized (solver) {
                ...
                synchronized (stoppingPoints) {
                    ...
                    if (stoppingPoints.get(robotID) != null)
        ...
        this.inference = new GatedThread("Coordinator inference") {
            ...
            public void runCore() {
                ...
                while (!stopInference) {
                    ...
                    if (CONTROL_PERIOD > 0) {
                        try {
                            GatedThread.sleep(Math.max(0, CONTROL_PERIOD - Calendar.getInstance().getTimeInMillis() + threadLastUpdate));
                        } catch (InterruptedException e) {
                            e.printStackTrace();
                        }
                    }
    ...
    public boolean isFree(int robotID) {
        ...
        synchronized (solver) {
            ...
            for (Pair<TrajectoryEnvelope, Long> te : missionsPool)
                if (te.getFirst().getRobotID() == robotID) return false;
            synchronized (trackers) {
                AbstractTrajectoryEnvelopeTracker tracker = trackers.get(robotID);
