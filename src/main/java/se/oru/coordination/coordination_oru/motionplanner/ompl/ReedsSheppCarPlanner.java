...
import se.oru.coordination.coordination_oru.motionplanner.ompl.ReedsSheppCarPlannerLib.PathPose;
import se.oru.coordination.coordination_oru.utility.GeometrySmoother;
import se.oru.coordination.coordination_oru.utility.GeometrySmoother.SmootherControl;
    ...
    public boolean doPlanning() {
        ...
        for (int i = 0; i < this.goal.length; i++) {
            ...
            if (this.om != null) {
                ...
                if (!INSTANCE.plan_multiple_circles(occ, w, h, res, mapOriginX, mapOriginY, robotRadius, xCoords, yCoords, numCoords, start_.getX(), start_.getY(), start_.getTheta(), goal_.getX(), goal_.getY(), goal_.getTheta(), path, pathLength, distanceBetweenPathPoints, turningRadius, planningTimeInSecs, algo.ordinal()))
                    return false;
            } else {
                if (!INSTANCE.plan_multiple_circles_nomap(xCoords, yCoords, numCoords, start_.getX(), start_.getY(), start_.getTheta(), goal_.getX(), goal_.getY(), goal_.getTheta(), path, pathLength, distanceBetweenPathPoints, turningRadius, planningTimeInSecs, algo.ordinal()))
                    return false;
            }
