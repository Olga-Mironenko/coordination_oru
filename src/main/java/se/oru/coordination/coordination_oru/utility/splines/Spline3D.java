package se.oru.coordination.coordination_oru.utility.splines;
...
public class Spline3D {
    ...
    public Spline3D(double[] points, Coordinate[] controlPoints, Type type) {
        ...
        for (int i = 0; i < points.length; i += 3)
            this.coords[i / 3] = new Coordinate(points[i], points[i + 1], points[i + 2]);
    }
    ...
    public PoseSteering[] asPoseSteerings() {
        ...
        for (int i = 0; i < poses.length; i++)
            pss[i] = new PoseSteering(poses[i].getX(), poses[i].getY(), poses[i].getTheta(), 0.0);
        return pss;
    }
