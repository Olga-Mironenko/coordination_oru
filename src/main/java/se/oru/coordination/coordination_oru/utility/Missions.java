...
public class Missions {
    ...
    public static Geometry getBackBlockingObstacle(TrajectoryEnvelope te) {
        Trajectory trajectory = te.getTrajectory();
        PoseSteering[] path = trajectory.getPoseSteering();
    ...
    public static void startMissionDispatchers(TrajectoryEnvelopeCoordinator tec, double lookAheadDistance,
        ...
        writeReports(tec, lookAheadDistance, writeReports, intervalInSeconds,
                        ...
                        for (int robotID : dispatchableRobots) {
                            if (Missions.hasMissions(robotID)) {
                                ...
                                if (m != null) {
                                    synchronized(tec) {
                                        if (tec.isFree(m.getRobotID())) {
                                            // Concatenate with future missions if necessary
                                            if (concatenatedMissions.containsKey(m)) {
                                                ArrayList<Mission> catMissions = concatenatedMissions.get(m);
     ...
     * @param resultDirectory The directory where to write the robot reports.
     */
    public static void writeReports(TrajectoryEnvelopeCoordinator tec, double lookAheadDistance,
                                    boolean writeReports, int intervalInSeconds, int terminationInMinutes,
                                    String heuristicName, String resultDirectory) {
     ...
     * @param tec The {@link TrajectoryEnvelopeCoordinator} that coordinates the missions.
     */
    public static void addRobotsForLooping(TrajectoryEnvelopeCoordinator tec) {
        for (int robotID : convertSetToIntArray(tec.getAllRobotIDs())) {
            dispatchableRobots.add(robotID);
     ...
     *
     * @param mission           The original mission.
     * @param catMissions The concatenated missions to be added.
     * @return The new concatenated mission.
     */
    private static Mission createConcatenatedMission(Mission mission, List<Mission> catMissions) {
        Mission lastMission = catMissions.get(catMissions.size() - 1);
        return new Mission(mission.getRobotID(), mission.getFromLocation(), lastMission.getToLocation(),
                mission.getFromPose(), lastMission.getToPose());
    }
     ...
     */
    private static void removeMissionAndConcatenatedMissions(Mission mission) {
        Missions.removeMissions(mission);
        System.out.println("Removed mission " + mission);
        List<Mission> concatenated = concatenatedMissions.get(mission);
