    ...
    public UPFAdapter(String[] init, String[] plan, String robotPrefix, String atPredicate, String movementAction) {
        ...
        for (String action : plan) {
            ...
            for (String arg : arguments) {
                if (arg.startsWith(robotPrefix))
                    robotsToIDs.put(arg, Integer.parseInt(arg.substring(robotPrefix.length())));
            }
        }
        ...
        for (String action : plan) {
            if (action.startsWith(movementAction)) {
                ...
                if (!motionPlanner.plan())
                    throw new Error("Could not plan path between " + arguments[1] + " and " + arguments[2] + " for robot_" + robotsToIDs.get(arguments[0]));
