        ...
        if (CREATE_SCENARIO) {
            ...
            for (int i = 0; i < numSlots; i++) {
                for (int j = 0; j < numSlots; j++) {
                    ...
                    if (!rsp.doPlanning())
                        throw new Error("No path between " + locationsUp.get(i) + "[" + Missions.getLocation(locationsUp.get(i)) + "] and " + locationsDown.get(j) + "[" + Missions.getLocation(locationsDown.get(j)) + "]");
