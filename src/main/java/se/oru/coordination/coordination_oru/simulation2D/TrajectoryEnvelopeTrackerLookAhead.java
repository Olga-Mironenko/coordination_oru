...
public abstract class TrajectoryEnvelopeTrackerLookAhead extends AbstractTrajectoryEnvelopeTracker implements Runnable {
	...
	public TrajectoryEnvelopeTrackerLookAhead(TrajectoryEnvelope te, int timeStep, double temporalResolution, TrajectoryEnvelopeCoordinator tec, TrackingCallback cb) {
		...
		TrajectoryEnvelopeTrackerLookAhead self = this;
		this.th = new Thread("LookAhead tracker " + te.getComponent()) {
			@Override
			public void run() {
				self.run();
			}
	...
	private void startInternalCPThread() {
		Thread t = new Thread("internalCPThread") {
			@Override
			public void run() {
	...
	private CriticalSection getFirstOfCurrentCriticalSections() {
		...
		var tec = TrajectoryEnvelopeCoordinatorSimulation.tec;
		for (CriticalSection cs : criticalSections) {
...
//			System.out.println(this.traj.getPose().length);
			...
			long deltaTimeInMillis = th.isAlive() ? delay : Calendar.getInstance().getTimeInMillis() - timeStart;
