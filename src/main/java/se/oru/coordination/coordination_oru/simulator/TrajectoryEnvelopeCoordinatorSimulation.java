...
import se.oru.coordination.coordination_oru.utility.CollisionEvent;
...
public class TrajectoryEnvelopeCoordinatorSimulation extends TrajectoryEnvelopeCoordinator {
    ...
    private ArrayList<Integer> computeStoppingPoints(PoseSteering[] poses) {
        ...
        if (poses.length > 1)
            prevTheta = Math.atan2(poses[1].getY() - poses[0].getY(), poses[1].getX() - poses[0].getX());
    ...
    public AbstractTrajectoryEnvelopeTracker getNewTracker(TrajectoryEnvelope te, TrackingCallback cb) {
        if (this.getRobotTrackingPeriodInMillis(te.getRobotID()) == null || this.getRobotMaxVelocity(te.getRobotID()) == null || this.getRobotMaxAcceleration(te.getRobotID()) == null)
            throw new Error("Robot" + te.getRobotID() + ": missing kinodynamic parameters.");
            ...
            collisionThread = new GatedThread("Collision checking thread.") {
                ...
                public void runCore() {
                    ...
                    while (true) {
                        ...
                        try {
                            GatedThread.sleep((long) (1000.0 / 30.0));
